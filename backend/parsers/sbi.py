...
def parse(text: str):
    ...
    card_variant = find_first(text, r"(Platinum|Elegance|Elite|Prime|SimplySAVE)", default="Unknown")
    billing = find_first(text, r"Billing Period[:\s]*([0-9/.\- ]+to[0-9/.\- ]+)")
    ...
    amount_due = find_first(text, r"(?:Total Amount Due|Amount Payable)[:\s]*₹?([\d,]+\.\d{2})")
    return build_base({
        "issuer": issuer,
