...
def parse(text: str):
    issuer = "AMEX"
    last4 = find_first(text, r"Card Ending[:\s]*(\d{4})", default=find_first(text, r"(\d{4})\b"))
    card_variant = find_first(text, r"(Platinum|Gold|Green|Centurion|Membership Rewards)", default="Unknown")
    ...
    amount_due = find_first(text, r"(?:New Balance|Total Amount Due|Amount Due)[:\s]*₹?([\d,]+\.\d{2})")
    return build_base({
        "issuer": issuer,
